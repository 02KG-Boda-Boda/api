...
class Members {
  ...
  static createMember(req, res) {
    const { firstName, lastName, nin, phoneNumber } = req.body;
    ...
    Member.create({
      ...
      .then((member) => {
        const welcomeMessage =
          ...
          " " +
          lastName +
          " Welcome to 2GO Financial Services. Your PIN is " +
          member.pin +
          " and phone number" +
          phoneNumber;
  ...
  static updateMember(req, res) {
    ...
    return Member.findByPk(req.params.member_id).then((member) => {
      if (!member) {
        return res.status(404).send({
