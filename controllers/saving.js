...
const { Saving, User, Member } = model;

class Savings {
  static debitSaving(req, res) {
    const { memberId, amount, phoneNumber } = req.body;
          ...
          return saving
            ...
            .then(() => {
              const welcomeMessage =
                "Your 2G0 Financial Services Account 2G00" +
                saving.id +
                " has been debited with UGX " +
                amount +
                ". Your Account Balance is UGX " +
                saving.amount;
  ...
  static createSaving(req, res) {
    const { memberId, amount, account_type, phoneNumber } = req.body;
    const userId = req.userData.userId;
    return Saving.findOne({
      where: {
        memberId: memberId,
        ...
        return Saving.create({
          ...
          .then((saving) => {
            const welcomeMessage =
              "Your 2G0 Financial Services Account 2G00" +
              saving.id +
              " has been created with an initial deposit of  UGX " +
              amount +
              ". Your Account Balance is UGX " +
              saving.amount;
            const phone = "+256" + phoneNumber.slice(1);
  ...
  static updateSaving(req, res) {
    const { memberId, amount, account_type } = req.body;

    return Saving.findByPk(req.params.saving_id).then((saving) => {
      if (!saving) {
